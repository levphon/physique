...
SERVER_NAME=`sed '/server.display-name/!d;s/.*=//' classes/application.yml | tr -d '\r'`
SERVER_PORT=`sed '/server.port/!d;s/.*=//' classes/application.yml | tr -d '\r'`
#LOGS_FILE=`sed '/log4j.appender.logToFile.File/!d;s/.*=//' classes/log4j.properties | tr -d '\r'`
LOGS_FILE=`sed '/logging.path/!d;s/.*=//' classes/application.yml | tr -d '\r'`
...
nohup $JAVA_HOME/bin/java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS -classpath $CONF_DIR:$LIB_JARS com.qurong.QurongApplication env=$ENV --spring.profiles.active=$ENV > $STDOUT_FILE 2>&1 &
